...
$Env:CONVERTPDFWITHLAYOUT_TELEMETRY_OPTIN = (-not $Evn:POWERSHELL_TELEMETRY_OPTOUT) # use the invert of default powershell telemetry setting

# Set up the telemetry
Initialize-THTelemetry -ModuleName "ExtractPDFData"
Set-THTelemetryConfiguration -ModuleName "ExtractPDFData" -OptInVariableName "CONVERTPDFWITHLAYOUT_TELEMETRY_OPTIN" -StripPersonallyIdentifiableInformation $true -Confirm:$false
...
Write-Information ("Telemetry for ExtractPDFData module is $(if([string] $Env:CONVERTPDFWITHLAYOUT_TELEMETRY_OPTIN -in ("no","false","0")){"NOT "})enabled. Change the behavior by setting the value of " + '$Env:CONVERTPDFWITHLAYOUT_TELEMETRY_OPTIN') -InformationAction Continue
